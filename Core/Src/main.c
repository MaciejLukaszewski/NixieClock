...
//mozliwe ze pwm leci do konca i dopiero potem sie wyłącza - sprawdzamie wartosci occr i robienie delay
void NixieLightUp(){
	HAL_ADC_Start_IT(&hadc);
	switch (Clock.NextNixie){
		case Nixie1 :			//turn on 1st Clock.NextNixie and turn off 4th one
			TIM1->CCR1 = ADC_val * 8 + 64 - ADC_val ;
		...
		case Nixie2 :			//turn on 2nd Clock.NextNixie and turn off 1st one
			TIM1->CCR2 = ADC_val * 8 + 64 - ADC_val;
		...
		case Nixie3 :			//turn on 3rd Clock.NextNixie and turn off 2nd one
			TIM1->CCR3 = ADC_val * 8 + 64 - ADC_val;
		...
		case Nixie4 :			//turn on 4th Clock.NextNixie and turn off 3rd one
			TIM1->CCR4 = ADC_val * 8 + 64 - ADC_val;
...
void TimeToNixie( TimeHolder *time, DisplayTime *TimeCont ){
	TimeCont->HoursUnity = time->Hours % 10;									// extract decimal value for tube 2
	TimeCont->HoursDecimal = (time->Hours - TimeCont->HoursUnity) / 10;  			// extract decimal value for tube 1
	TimeCont->MinutesUnity = time->Minutes % 10;            					// extract decimal value for tube 4
	TimeCont->MinutesDecimal = (time->Minutes - TimeCont->MinutesUnity) / 10;		// extract decimal value for tube 3
}
...
int main(void)
{
  ...
  while (1){
	  ...
	  Clock.ClockTime.Hours = sTime.Hours;							// get the hours		CHANGE FOR MINUTES AND HOURS NOT SECONDS
	  Clock.ClockTime.Minutes = sTime.Minutes;						// get the minutes

	  /*
	   * Write value to the variable which is displayed
	   */
	  ...
	  /*
	   * Control which tube to light up
	   */
	  if( Clock.DispMode == ContinuouseMode){
		  if( Clock.Tube == None ){			//if exit MoveMode and Tube == None
			  Clock.Tube = All;
		  }
	  }
	  else{	//MoveMode
	  ...
	  /*
	   * Nixie PWM control - IRQ handling - DISPLAY
	   */
	  if(timerFlag){
		  timerFlag = 0;
		  NixieLightUp();
	  ...
	  /*
	  * Button B0 press handling
	  */
	  if(B0.flag){								//if interrupt occurs on this button
	  		  ...
	  		  case Hold:
	  			switch(Clock.ClockState){
					...
					case TimeSet:
						...
						//write to time REGISTER set time
						set_time(Clock.ClockTime.Hours, Clock.ClockTime.Minutes, 0);

						break;
					case AlarmSet:
						Clock.ClockState = Alarm;
						NixieToTime(&SettingTim, &Clock.AlarmTime);
						//write to alarm REGISTER set alarm
						set_alarm(Clock.AlarmTime.Hours, Clock.AlarmTime.Minutes, 0);
						break;
	  			}
	  			  break;
	  		  default:
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, NixieDigitA_Pin|NixieDigitB_Pin|NixieDigitC_Pin|NixieDigitD_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : NixieDigitA_Pin NixieDigitB_Pin NixieDigitC_Pin NixieDigitD_Pin */
  GPIO_InitStruct.Pin = NixieDigitA_Pin|NixieDigitB_Pin|NixieDigitC_Pin|NixieDigitD_Pin;
